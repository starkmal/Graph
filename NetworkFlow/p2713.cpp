...
int Sap(int x,int flow){
	...
	for(i=Last[x];i;i=Next[i])
		if(V[i]&&Dis[u=End[i]]+1==Dis[x]){
			...
			if(d==flow||Dis[S]>=Max)return d;
		}
	if(Dis[S]>=Max)return d;
	if(--Cnt[Dis[x]]==0)Dis[S]=Max;
	Cnt[++Dis[x]]++;
	return d;
}
int main(){
	...
	S=n+1,T=S+1,EX=Tote+1,Max=T;
	for(i=1;i<=n;i++)
		if(Lim[i]>=0)Ins(S,i,Lim[i]),low+=Lim[i];
		else Ins(i,T,-Lim[i]);
	Ins(n,1,Inf);

	while(Dis[S]<Max)Sap(S,Inf);
	ans=V[Tote];
